...
// Fallback logic to render basic home content
function renderApp() {
  const appEl = $("#app");
  if (!appEl) return;
    ...
    appEl.innerHTML = `
      <section class="hero">
        <div class="hero-copy">
          ...
          <h1>See what pets<br><span style="color:red">are worth right now.</span></h1>
          <p>A simple value list for Pet Simulator X.</p>
        </div>
        <div class="hero-stats">
          <div class="stat"><b>${pets.length}</b><span>Total Pets</span></div>
          <div class="stat"><b>${exclusiveCount}</b><span>Exclusives</span></div>
        </div>
      </section>
      <section class="pet-list" style="display:grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap:20px; padding:20px;">
        ${pets.map(p => `
          <div class="pet-card" style="border:1px solid #ccc; padding:15px; border-radius:8px;">
            <h3>${p.emoji} ${esc(p.name)}</h3>
            <p>Value: ${fmt(p.value)}</p>
            <p>Demand: ${p.demand}/10</p>
          </div>
        `).join('')}
      </section>
    `;
  } else {
    appEl.innerHTML = `<section style="padding:20px;"><h2>${esc(currentRoute.toUpperCase())} Page</h2><p>Content coming soon.</p></section>`;
  }
}
